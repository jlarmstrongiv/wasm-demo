...
let sclblRuntime = (function () {
  ...
  sclbl.checkSettings = function () {
    ...
    if (
      sclbl.settings.cfid.substring(sclbl.settings.cfid.length - 5) === ".wasm"
    ) {
  ...
  sclbl.run = function (rawInput) {
    ...
    return new Promise(async function (resolve, reject) {
      // note, async not supported by JSHint
      ...
      if (sclbl.settings.location === "local") {
        ...
        try {
          ...
          if (!sclbl.settings.strict) {
            // fallback to remote if !strict
            try {
              input = sclbl.convertRemoteInput(
                rawInput,
                sclbl.settings.inputType
              );
      ...
      if (sclbl.settings.location === "remote") {
        ...
        try {
          input = await sclbl.convertRemoteInput(
            rawInput,
            sclbl.settings.inputType
          );
